...
Wallet::Wallet (const Wallet &other)
{
  ...
  coins = other.coins;
  blockedDollars = other.blockedDollars;
  blockedCoins = other.blockedCoins;
}
...
void Wallet::depositDollars(double d) // esto tiene un error por que no checa si es positivo 
{
  if (d < 0)
  {
    return;
...
void Wallet::depositCoins(double c) // esto tiene un error por que no checa si es positivo
{
  if (c < 0)
  {
    return;
...
void Wallet::withdrawDollars(double d) // no checa si es positiva, por lo que puede dar negativo
{
  if (d < 0)
  {
    return;
